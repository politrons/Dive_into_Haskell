...
startAdventure adventureInfoRef = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
...
createCharacter adventureInfoRef = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
...
processAction adventureInfoRef = do
  action <- extractUriParam "action"
  let playerActions = splitOn " " action
  timeline <- toActionM $ getTimeLineState adventureInfoRef
  let puzzleResult = processActionsInChapterActions timeline playerActions
...
processActionsInChapterActions timeline playerActions =
  case maybeActions of
    Just actions -> not (null list)
      where list = playerActions >>= \playerAction -> filter (\chapterAction -> playerAction == chapterAction) actions
    Nothing -> False
  where
...
processChapterResult puzzleResult adventureInfoRef =
  case puzzleResult of
    True -> do
      adventureInfoRef <- updatePlayerToNextLevel adventureInfoRef
      getStoryPage adventureInfoRef chapterPages
    False -> do
      adventureInfoRef <- updatePlayerToError adventureInfoRef
      maxAttemptsReached <- processNumberOfErrors adventureInfoRef
      page <-
        case maxAttemptsReached of
          True -> do
            errorPage <- getStoryPage adventureInfoRef chapterGameOverPages
            return errorPage
          False -> do
            errorPage <- getStoryPage adventureInfoRef chapterErrorPages
            return errorPage
      return page
...
processNumberOfErrors adventureInfoRef = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
...
updatePlayerInfo name raceMaybe adventureInfoRef =
  case raceMaybe of
    Just race -> do
      _ <- writeIORef adventureInfoRef (AdventureInfo (PlayerInfo name race) (TimeLine 1 (Attempts 0)))
      ...
      return $ replaceRaceNameByImageInStory story (raceName race)
    Nothing -> readStoryTimeLine "error.html"
...
updatePlayerToNextLevel adventureInfoRef = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
  _ <-
    writeIORef
      adventureInfoRef
      (AdventureInfo (playerInfo adventureInfo) (TimeLine (state (timeline adventureInfo) + 1) (Attempts 0)))
...
updatePlayerToError adventureInfoRef = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
  _ <-
    writeIORef
      adventureInfoRef
      (AdventureInfo
         (playerInfo adventureInfo)
         (TimeLine (state (timeline adventureInfo)) (Attempts (number (attempts (timeline adventureInfo)) + 1))))
  return adventureInfoRef
...
replaceRaceNameByImageInStory story raceName =
  case raceName of
...
getStoryPage adventureInfoRef collectionPage = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
  return $ fromMaybe "error.html" (Map.lookup (state (timeline adventureInfo)) collectionPage)

{-| Function to find the timeline of your adventure-}
getTimeLineState :: IORef AdventureInfo -> IO TimeLine
getTimeLineState adventureInfoRef = do
  adventureInfo <- liftIO $ readIORef adventureInfoRef
  return $ timeline adventureInfo

{-| Function to read the html page and transform in Text|-}
readStoryTimeLine :: String -> IO Text
readStoryTimeLine page = do
  fileContent <- readFile $ "src/programs/adventure/story/" ++ page
...
chapterErrorPages =
  Map.fromList [(1, "storyError1.html"), (2, "storyError2.html"), (3, "storyError3.html"), (4, "storyError4.html")]

chapterGameOverPages :: Map Int String
chapterGameOverPages =
  Map.fromList [(1, "storyOver1.html"), (2, "storyOver2.html"), (3, "storyOver3.html"), (4, "storyOver4.html")]

{-| ----------------------------------------------}
{-|                    MODEL                     -}
{-| ----------------------------------------------}
newtype Attempts = Attempts
  { number :: Int
  }
...
humanImage =
  "https://static.squarespace.com/static/51b3dc8ee4b051b96ceb10de/51ce6099e4b0d911b4489b79/51ce619ce4b0d911b449a17f/1316719790027/1000w/ranger_and_gondorian_by_otisframpton-d3crwha.jpeg"

wizardImage =
  "https://static.squarespace.com/static/51b3dc8ee4b051b96ceb10de/51ce6099e4b0d911b4489b79/51ce619ce4b0d911b449a17e/1316719755098/1000w/wizard_and_hobbit_by_otisframpton-d3crwd0.jpeg"

elfImage =
  "https://static.squarespace.com/static/51b3dc8ee4b051b96ceb10de/51ce6099e4b0d911b4489b79/51ce619ce4b0d911b449a180/1316719808523/1000w/elf_and_dwarf_by_otisframpton-d3crwmm.jpeg"
